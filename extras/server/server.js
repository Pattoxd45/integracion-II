...
const options = {
  key: fs.readFileSync(
    "/etc/letsencrypt/archive/magicarduct.online/privkey1.pem",
  ),
  cert: fs.readFileSync(
    "/etc/letsencrypt/archive/magicarduct.online/fullchain1.pem",
  ),
};
...
app.post("/login", (req, res) => {
  ...
  if (!email || !password) {
    return res
      .status(400)
      .json({ message: "Correo y contraseï¿½a son requeridos." });
  }
  ...
  db.query(query, [email, password], (err, results) => {
    ...
    if (results.length === 0) {
      return res
        .status(401)
        .json({ message: "Correo o contraseï¿½a incorrectos." });
    }
    ...
    res
      .status(200)
      .json({
        message: "Inicio de sesiï¿½n exitoso",
...
app.post("/register", (req, res) => {
  ...
  db.query(checkQuery, [correo], (err, results) => {
    ...
    const query =
      "INSERT INTO usuario (nombre, correo, clave, foto) VALUES (?, ?, ?, 1)";
...
app.post("/api/createmazo", (req, res) => {
  ...
  const queryMazo =
    "INSERT INTO barajas (nombre, formato, descripcion) VALUES (?, ?, ?)";
  db.query(queryMazo, [nombre, formato, descripcion], (err, result) => {
    ...
    const queryUsuarioMazo =
      "INSERT INTO barajas_de_usuario (id_usuario, idbarajas_de_usuario) VALUES (?, ?)";
    db.query(queryUsuarioMazo, [idusuario, nuevoMazoId], (err) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "Error al asociar el mazo al usuario" });
      }
...
app.put("/api/actualizarmazo/:id", (req, res) => {
  ...
  const query =
    "UPDATE barajas SET nombre = ?, formato = ?, descripcion = ? WHERE idbarajas = ?";
...
app.delete("/api/eliminarmazo/:id", (req, res) => {
  ...
  db.query(deleteMazoCartasQuery, [mazoId], (err, result) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "Error al eliminar las referencias en mazo_cartas" });
    }

    // Luego, eliminamos las referencias en la tabla de relaciÃ³n barajas_de_usuario
    const deleteRelacionQuery =
      "DELETE FROM barajas_de_usuario WHERE idbarajas_de_usuario = ?";
    db.query(deleteRelacionQuery, [mazoId], (err, result) => {
      if (err) {
        console.error(
          "Error al eliminar las referencias del mazo en barajas_de_usuario:",
          err,
        );
        return res
          .status(500)
...
app.delete("/api/eliminarmazo2/:nombre/:idUsuario", (req, res) => {
  ...
  if (!nombre || !idUsuario) {
    return res
      .status(400)
      .json({ error: "El nombre del mazo y el ID del usuario son requeridos" });
  }
  ...
  db.query(getMazoIdQuery, [nombre, idUsuario], (err, results) => {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ error: "Mazo no encontrado para el usuario especificado" });
    }
...
app.post("/api/mazocartas", (req, res) => {
  ...
  const query =
    "INSERT INTO mazo_cartas (IDmazo, IDcarta, cantidad) VALUES (?, ?, ?)";
...
app.put("/api/actualizarmazocartas/:idmazo/:idcarta", (req, res) => {
  ...
  const query =
    "UPDATE mazo_cartas SET cantidad = ? WHERE IDmazo = ? and IDcarta = ?";
...
app.delete("/api/eliminarmazocarta/:idmazo/:idcarta", (req, res) => {
  ...
  if (!idmazo || !idcarta) {
    return res
      .status(400)
      .json({ error: "Faltan datos para eliminar la carta" });
  }
  ...
  db.query(query, [idmazo, idcarta], (err, result) => {
    ...
    if (result.affectedRows === 0) {
      return res
        .status(404)
        .json({ message: "Carta no encontrada en el mazo" });
    }
...
app.post("/api/cartasfavoritas", (req, res) => {
  ...
  const countQuery =
    "SELECT COUNT(*) AS cartaCount FROM cartas_favoritas WHERE IDusuario = ?";
  db.query(countQuery, [IDusuario], (err, countResult) => {
    if (err) {
      console.error("Error al contar cartas favoritas:", err);
      return res
        .status(500)
        .json({ error: "Error al contar cartas favoritas" });
    }
    ...
    if (cartaCount >= 5) {
      ...
      db.query(deleteQuery, [IDusuario], (err) => {
        if (err) {
          console.error(
            "Error al eliminar la carta favorita más antigua:",
            err,
          );
          return res
            .status(500)
            .json({ error: "Error al eliminar la carta favorita más antigua" });
        }
    ...
    db.query(reorderQuery, [IDusuario], (err) => {
      ...
      db.query(insertQuery, [IDusuario, IDcarta], (err) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ error: "Error al agregar nueva carta favorita" });
        }

        console.log(
          "Nueva carta favorita agregada y IDnumero reordenados exitosamente",
        );
        res
          .status(200)
          .json({
            message: "Nueva carta favorita agregada y IDnumero reordenados",
          });
      });
    });
  });
...
app.get("/api/cartasfavoritas/:idusuario", (req, res) => {
  ...
  db.query(selectQuery, [idusuario], (err, results) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "Error al obtener cartas favoritas" });
    }
    ...
    if (results.length === 0) {
      ...
      return res
        .status(404)
        .json({ message: "No se encontraron cartas favoritas" });
    }
...
app.delete("/api/cartasfavoritas", (req, res) => {
  ...
  const deleteQuery =
    "DELETE FROM cartas_favoritas WHERE IDusuario = ? AND IDcarta = ?";
  db.query(deleteQuery, [idusuario, idcarta], (err, result) => {
    ...
    if (result.affectedRows === 0) {
      ...
      return res
        .status(404)
        .json({ error: "La carta no se encontró en la lista de favoritas" });
    }
    ...
    db.query(reorderQuery, [idusuario], (err) => {
      if (err.code === "ER_WARN") {
        ...
        return res
          .status(500)
          .json({ error: "Error al reordenar los IDnumero" });
      }
      ...
      res
        .status(200)
        .json({
          message: "Carta eliminada y IDnumero reordenados exitosamente",
        });
    });
  });
});
...
app.post("/api/ultimascartasvistas", (req, res) => {
  ...
  const countQuery =
    "SELECT COUNT(*) AS cartaCount FROM ultimas_cartas_vistas WHERE IDusuario = ?";
  db.query(countQuery, [IDusuario], (err, countResult) => {
    if (err) {
    ...
    if (cartaCount >= 5) {
      ...
      db.query(deleteQuery, [IDusuario], (err) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ error: "Error al eliminar la carta mÃ¡s antigua" });
        }
    ...
    db.query(reorderQuery, [IDusuario], (err) => {
      ...
      db.query(insertQuery, [IDusuario, IDcarta], (err) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ error: "Error al agregar nueva carta" });
        }

        console.log("Nueva carta agregada y IDnumero reordenados exitosamente");
        res
          .status(200)
          .json({ message: "Nueva carta agregada y IDnumero reordenados" });
      });
    });
  });
});
...
app.get("/api/ultimascartasvistas/:IDusuario", (req, res) => {
  ...
  const query =
    "SELECT * FROM ultimas_cartas_vistas WHERE IDusuario = ? ORDER BY IDnumero DESC";
  db.query(query, [IDusuario], (err, results) => {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ message: "No se encontraron cartas vistas para este usuario" });
    }
