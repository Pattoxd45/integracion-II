...
const Cartas = () => {
  ...
  const filteredCards = cards.filter((card) => {
    ...
    // Si no hay un filtro seleccionado, mostrar todas las cartas
    if (selectedFilter === "none" || filterValue === "all") {
      return matchesSearch; // Solo aplicar búsqueda
    }
    ...
    <div className="max-w-[1200px] mx-auto my-6">
      ...
      <div className="flex flex-col mb-6 space-y-4">
        ...
        <div className="flex space-x-4">
          ...
          {/* Valor del Filtro, depende de lo seleccionado */}
          <select
            value={filterValue}
            onChange={(e) => setFilterValue(e.target.value)}
            className="p-2 border-2 border-gray-400 rounded-md"
            disabled={selectedFilter === "none"} // Desactivar si no se ha seleccionado un filtro
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-5 justify-items-center">
        {filteredCards.length > 0 ? (
          filteredCards.map((card, index) => (
            ...
            >
              {/* Círculo con el coste en la esquina superior derecha */}
