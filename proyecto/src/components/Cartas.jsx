...
const Cartas = () => {
  ...
  const [filter, setFilter] = useState({ order: 'name', dir: 'auto' });
  ...
  const fetchCards = () => {
    setLoading(true);
    fetch(`https://api.scryfall.com/cards/search?q=${encodeURIComponent(searchQuery)}&order=${filter.order}&dir=${filter.dir}`)
  ...
  return (
    <div className="p-6 bg-gray-900 min-h-screen">
      ...
      <div className="mb-4">
        ...
        />
        <select onChange={handleOrderChange} className="ml-2 p-2 rounded border border-gray-500">
